...
FUNCTION( CONFIGURE_BLAS_AND_LAPACK )
    ...
    IF( USE_MATLAB AND USE_MATLAB_LAPACK ) 
        ...
        SET( CMAKE_REQUIRED_LIBRARIES ${MATLAB_LAPACK} )
        ...
        ENDIF()
        SET( BLAS_LAPACK_LIBS ${MATLAB_LAPACK} PARENT_SCOPE )
    ...
    ELSEIF( USE_OPENBLAS ) 
        ...
        SET( BLAS_LAPACK_LIBS ${OPENBLAS_LIBS} PARENT_SCOPE)
    ELSE()
        IF ( USE_EXT_BLAS )
            SET( USE_BLAS 1 )
        ENDIF()
...
FUNCTION( CONFIGURE_VECLIB )
    ...
    FIND_LIBRARY( BLAS_LIBRARY    NAMES BLAS    PATHS "${VECLIB_DIRECTORY}"  NO_DEFAULT_PATH )
    FIND_LIBRARY( LAPACK_LIBRARY  NAMES LAPACK  PATHS "${VECLIB_DIRECTORY}"  NO_DEFAULT_PATH )
...
# Macro to configure OPENBLAS
FUNCTION( CONFIGURE_OPENBLAS )
    IF ( NOT OPENBLAS_DIRECTORY )
        MESSAGE(FATAL_ERROR "Default search for OpenBLAS not supported, set OPENBLAS_DIRECTORY" )
    ENDIF()
    VERIFY_PATH( ${OPENBLAS_DIRECTORY} )
    VERIFY_PATH( ${OPENBLAS_DIRECTORY}/include )
    VERIFY_PATH( ${OPENBLAS_DIRECTORY}/lib )
    FILE(APPEND ${BLAS_LAPACK_HEADER} "#include \"${OPENBLAS_DIRECTORY}/include/cblas.h\"\n" )
    FILE(APPEND ${BLAS_LAPACK_HEADER} "#include \"${OPENBLAS_DIRECTORY}/include/lapacke.h\"\n" )
    FIND_LIBRARY( OPENBLAS_LIBRARY    NAMES openblas    PATHS "${OPENBLAS_DIRECTORY}/lib"  NO_DEFAULT_PATH )
    SET( OPENBLAS_LIBS ${OPENBLAS_LIBRARY} )
    MESSAGE( "Using OpenBLAS" )
    MESSAGE( "   ${OPENBLAS_LIBS}" )
ENDFUNCTION()

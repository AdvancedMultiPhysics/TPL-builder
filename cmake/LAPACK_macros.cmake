...
FUNCTION( CHECK_MKL INSTALL_PATH )
    ...
    IF ( ${SYSTEM_NAME} STREQUAL "Windows_32" OR ${SYSTEM_NAME} STREQUAL "Windows_64" )
        SET( MKL_LINK "${MKL_LIBS}" )
    ELSE()
        SET( MKL_LINK "-Wl,--no-as-needed -Wl,--start-group ${MKL_LIBS} -Wl,--end-group -lpthread -lm" )
    ENDIF()
    STRING( REPLACE ";" " " MKL_LINK "${MKL_LINK}" )
...
FUNCTION( CHECK_ONEAPI INSTALL_PATH )
    ...
    ELSE()
        ...
        ENDFOREACH()
        IF ( ${SYSTEM_NAME} STREQUAL "Windows_32" OR ${SYSTEM_NAME} STREQUAL "Windows_64" )
            SET( MKL_LINK "${MKL_LIBS} ${MKL_LINK_LIBRARIES}" )
        ELSE()
            SET( MKL_LINK "-Wl,--no-as-needed -Wl,--start-group ${MKL_LIBS} -Wl,--end-group ${MKL_LINK_LIBRARIES} -lpthread -lm" )
        ENDIF()
        STRING( REPLACE ";" " " MKL_LINK "${MKL_LINK}" )
        SET( LAPACK_VENDOR "MKL" PARENT_SCOPE )
        SET( BLAS_FOUND true PARENT_SCOPE )
...
FUNCTION( CHECK_DEFAULT INSTALL_PATH )
    ...
    ENDIF()
    IF ( ${SYSTEM_NAME} STREQUAL "Windows_32" OR ${SYSTEM_NAME} STREQUAL "Windows_64" )
        SET( BLAS_LAPACK_LINK "${BLAS_LIBS} ${LAPACK_LIBS}" )
    ELSE()

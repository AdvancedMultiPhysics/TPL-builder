...
FUNCTION( CALL_ADD_TEST EXEFILE )
    # Print the call command (useful for debugging)
    # MESSAGE( "CALL_ADD_TEST ${EXEFILE} ${ARGN}" )
    ...
    SET( optionalArgs WEEKLY TESTBUILDER )
    SET( oneValueArgs TESTNAME PROCS THREADS )
    SET( multiValueArgs RESOURCES ARGS )
    CMAKE_PARSE_ARGUMENTS( CALL_ADD_TEST "${optionalArgs}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN} )
    SET( TESTNAME ${CALL_ADD_TEST_TESTNAME} )
    SET( THREADS ${CALL_ADD_TEST_THREADS} )
    SET( PROCS ${CALL_ADD_TEST_PROCS} )
    SET( WEEKLY ${CALL_ADD_TEST_WEEKLY} )
    SET( TESTBUILDER ${CALL_ADD_TEST_TESTBUILDER} )
    SET( RESOURCES ${CALL_ADD_TEST_RESOURCES} )
    ...
    IF ( NOT TESTNAME )
        ...
        ENDIF()
        CREATE_TEST_NAME( ${TESTNAME} ${ARGS} )
    ENDIF()
    ...
    ELSEIF ( ( ${PROCS} STREQUAL "1" ) AND NOT USE_MPI_FOR_SERIAL_TESTS )
        IF ( ${TESTBUILDER} )
            ADD_TEST( NAME ${TESTNAME} COMMAND $<TARGET_FILE:${TB_TARGET}> ${EXEFILE} ${ARGS} )
        ELSE()
            ADD_TEST( NAME ${TESTNAME} COMMAND $<TARGET_FILE:${LAST_TEST}> ${ARGS} )
        ENDIF()
    ELSEIF ( USE_MPI OR USE_EXT_MPI )
        IF ( ${TESTBUILDER} )
            ADD_TEST( NAME ${TESTNAME} COMMAND ${MPIEXEC} ${MPIEXEC_NUMPROC_FLAG} ${PROCS} $<TARGET_FILE:${TB_TARGET}> ${EXEFILE} ${ARGS} )
        ELSE()
            ADD_TEST( NAME ${TESTNAME} COMMAND ${MPIEXEC} ${MPIEXEC_NUMPROC_FLAG} ${PROCS} $<TARGET_FILE:${LAST_TEST}> ${ARGS} )
        ENDIF()
    ...
    # Add resource locks
    IF( RESOURCES )
        ADD_RESOURCE( ${TESTNAME} ${RESOURCES} )
    ELSE()
        ADD_RESOURCE_LOCKS( ${TESTNAME} ${EXEFILE} ${ARGS} )
    ENDIF()
ENDFUNCTION()
...
ENDMACRO()
MACRO( ADD_${PROJ}_TEST_THREADED EXEFILE THREADS ${ARGN} )
    CALL_ADD_TEST( ${EXEFILE} THREADS ${THREADS} ${ARGN} )
ENDMACRO()
MACRO( ADD_${PROJ}_TEST_PARALLEL EXEFILE PROCS ${ARGN} )
    CALL_ADD_TEST( ${EXEFILE} PROCS ${PROCS} ${ARGN} )
ENDMACRO()
MACRO( ADD_${PROJ}_WEEKLY_TEST EXEFILE PROCS ${ARGN} )
    CALL_ADD_TEST( ${EXEFILE} PROCS ${PROCS} WEEKLY ${ARGN} )
ENDMACRO()
MACRO( ADD_${PROJ}_TEST_THREAD_MPI EXEFILE PROCS THREADS ${ARGN} )
    CALL_ADD_TEST( ${EXEFILE} PROCS ${PROCS} THREADS ${THREADS} ${ARGN} )
ENDMACRO()
...
MACRO( ADD_TB_TEST_PARALLEL EXEFILE PROCS ${ARGN} )
    CALL_ADD_TEST( ${EXEFILE} PROCS ${PROCS} TESTBUILDER ${ARGN} )
ENDMACRO()
MACRO( ADD_TB_TEST_THREADED EXEFILE THREADS ${ARGN} )
    CALL_ADD_TEST( ${EXEFILE} THREADS ${THREADS} TESTBUILDER ${ARGN} )
ENDMACRO()
MACRO( ADD_TB_WEEKLY_TEST EXEFILE PROCS ${ARGN} )
    CALL_ADD_TEST( ${EXEFILE} PROCS ${PROCS} WEEKLY TESTBUILDER ${ARGN} )
ENDMACRO()
MACRO( ADD_TB_TEST_THREAD_MPI EXEFILE PROCS THREADS ${ARGN} )
    CALL_ADD_TEST( ${EXEFILE} PROCS ${PROCS} THREADS ${THREADS} TESTBUILDER ${ARGN} )
ENDMACRO()

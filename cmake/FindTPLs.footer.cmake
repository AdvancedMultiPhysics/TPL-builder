...
FOREACH ( tmp C CXX Fortran CUDA HIP )
    IF ( CMAKE_${tmp}_COMPILER_ID )
        FILE( APPEND "${TPLs_HEADER}" "#define ${PROJ}_${tmp}_COMPILER \"${CMAKE_${tmp}_COMPILER_ID}\"\n")
    ENDIF()
    IF ( CMAKE_${tmp}_STANDARD )
        FILE( APPEND "${TPLs_HEADER}" "#define ${PROJ}_${tmp}_STANDARD ${CMAKE_${tmp}_STANDARD}\n")
    ENDIF()
ENDFOREACH()
FOREACH ( tmp GPU_AWARE_MPI )
    IF ( ENABLE_${tmp} )
        FILE( APPEND "${TPLs_HEADER}" "#define ${PROJ}_ENABLE_${tmp}\n")
    ENDIF()
ENDFOREACH()
FOREACH ( tmp CUDA HIP OPENMP OPENACC OPENCL GPU_AWARE_MPI )
    IF ( USE_${tmp} )
        FILE( APPEND "${TPLs_HEADER}" "#define ${PROJ}_USE_${tmp}\n")
    ENDIF()
ENDFOREACH()
IF ( USE_CUDA OR USE_HIP )
    FILE( APPEND "${TPLs_HEADER}" "#define ${PROJ}_USE_DEVICE\n")
ENDIF()
FOREACH( TPL ${TPLs_LIST} )

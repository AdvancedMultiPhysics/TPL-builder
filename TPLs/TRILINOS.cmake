...
# Configure optional/required TPLs -- KOKKOSKERNELS will be required for v16 onwards
IF ( CMAKE_BUILD_TRILINOS )
    CONFIGURE_DEPENDENCIES( TRILINOS REQUIRED LAPACK KOKKOS OPTIONAL KOKKOSKERNELS BOOST HDF5 NETCDF )
ELSE()
    CONFIGURE_DEPENDENCIES( TRILINOS REQUIRED  OPTIONAL LAPACK KOKKOS KOKKOSKERNELS BOOST HDF5 NETCDF )
ENDIF()
...
SET( TRILINOS_OPTS "${TRILINOS_OPTS};-DCMAKE_CXX_FLAGS_DEBUG=${CMAKE_CXX_FLAGS_DEBUG}" )
SET( TRILINOS_OPTS "${TRILINOS_OPTS};-DCMAKE_CXX_FLAGS_RELEASE=${CMAKE_CXX_FLAGS_RELEASE}" )
...
IF ( CMAKE_BUILD_TRILINOS )
    ...
    ENDFOREACH()
    # Check shared vs static libraries
    IF ( ENABLE_SHARED )
        WRITE_TRILINOS_CONFIG_BOOL( BUILD_SHARED_LIBS ON )
    ELSE()
        WRITE_TRILINOS_CONFIG_BOOL( BUILD_SHARED_LIBS OFF )
    ENDIF()
    # Configure kokkos
    ...
    ENDIF()
    SET( LAPACK_LIBS2 ${BLAS_LAPACK_LINK} )
    SET( BLAS_LIBS2 ${BLAS_LAPACK_LINK} )
    ...
    WRITE_TRILINOS_CONFIG_BOOL( HAVE_dggsvd3_POST ON )
    # Configure MPI
    IF ( USE_MPI )
    ...
    IF ( TRILINOS_PACKAGES )
        ...
        IF ( ( EPETRA_INDEX GREATER "-1" ) AND ( XPETRA_INDEX GREATER "-1" ) )
            ...
            WRITE_TRILINOS_CONFIG_BOOL( Epetra_INST_INT_INT ON )
            # next line required by Trilinos 16 at least when Epetra is enabled
            WRITE_TRILINOS_CONFIG_BOOL( Tpetra_INST_INT_INT ON )
        ENDIF()
        IF ( ( TPETRA_INDEX GREATER "-1" ) AND ( XPETRA_INDEX GREATER "-1" ) )
            WRITE_TRILINOS_CONFIG_BOOL( Xpetra_ENABLE_Tpetra ON )
            WRITE_TRILINOS_CONFIG_BOOL( Tpetra_INST_INT_LONG_LONG OFF )
            # Commented till we transition from Epetra
#            WRITE_TRILINOS_CONFIG_BOOL( Tpetra_INST_INT_LONG ON )
        ENDIF()
    ELSE()
...
FILE( APPEND "${FIND_TPLS_CMAKE}" "IF ( TPLs_FIND_TRILINOS AND NOT TPLs_TRILINOS_FOUND )\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "    FIND_PACKAGE(Trilinos REQUIRED PATHS \"${TRILINOS_INSTALL_DIR}\" )\n" )
...
FILE( APPEND "${FIND_TPLS_CMAKE}" "    SET( TRILINOS_INCLUDE_DIR $\{Trilinos_INCLUDE_DIRS} )\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "    SET( TRILINOS_INSTALL_DIR $\{Trilinos_INSTALL_DIR} )\n" )
IF ( TRILINOS_KOKKOS )
    FILE( APPEND "${FIND_TPLS_CMAKE}" "    SET( TRILINOS_INCLUDE_DIR $\{TRILINOS_INCLUDE_DIR} ${TRILINOS_INSTALL_DIR}/include/kokkos )\n" )
ENDIF()
FILE( APPEND "${FIND_TPLS_CMAKE}" "    ADD_TPL_LIBRARY( TRILINOS $\{Trilinos_LIBRARIES} )\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "ENDIF()\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "\n" )

...
CONFIGURE_TPL( LAPACK )
CONFIGURE_OPTIONAL_TPL( BOOST )
CONFIGURE_OPTIONAL_TPL( HDF5 )
CONFIGURE_OPTIONAL_TPL( NETCDF )


# Configure Kokkos
CONFIGURE_TPL( KOKKOS )
IF ( KOKKOS_CMAKE_URL OR KOKKOS_CMAKE_SOURCE_DIR )
    SET( TRILINOS_DEPENDS ${TRILINOS_DEPENDS} KOKKOS )
ENDIF()


# Initialize some basic variables
SET( TRILINOS_DEPENDS KOKKOS LAPACK )
IF ( BOOST_CONFIGURED )
    SET( TRILINOS_DEPENDS BOOST )
ENDIF()
IF ( HDF5_CONFIGURED )
    SET( TRILINOS_DEPENDS HDF5 )
ENDIF()
IF ( NETCDF_CONFIGURED )
    SET( TRILINOS_DEPENDS NETCDF )
ENDIF()
...
IF ( CMAKE_BUILD_TRILINOS )
    ...
    IF ( TRILINOS_PACKAGES )
        ...
        ENDIF()
        IF ( KOKKOS_USE_CUDA )
            SET( TRILINOS_OPTS ${TRILINOS_OPTS} -DTPL_ENABLE_CUDA:BOOL=ON )
            SET( TRILINOS_OPTS ${TRILINOS_OPTS} -DTpetra_ENABLE_CUDA:BOOL=ON )
            SET( TRILINOS_OPTS ${TRILINOS_OPTS} -DTpetra_INST_CUDA:BOOL=ON )
        ENDIF()

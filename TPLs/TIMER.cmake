...
IF ( CMAKE_BUILD_TIMER )
    ...
    ENDIF()
    IF ( USE_MPI )
        SET( CONFIGURE_OPTIONS "${CONFIGURE_OPTIONS};-DMPI_COMPILER=${DMPI_COMPILER}" )
        SET( CONFIGURE_OPTIONS "${CONFIGURE_OPTIONS};-DMPIEXEC=${MPIEXEC_EXECUTABLE}" )
    ENDIF()
    IF ( ENABLE_TIMERUTILITY_GUI )
        SET( CONFIGURE_OPTIONS "${CONFIGURE_OPTIONS};-DENABLE_GUI=1" )
        IF ( QT_VERSION )
            SET( CONFIGURE_OPTIONS "${CONFIGURE_OPTIONS};-DQT_VERSION=${QT_VERSION}" )
        ELSE()
            MESSAGE(FATAL_ERROR "Please specify QT_VERSION and QWT_URL")
        ENDIF()
        IF ( QWT_URL )
            SET( CONFIGURE_OPTIONS "${CONFIGURE_OPTIONS};-DQWT_URL=${QWT_URL}" )
        ELSE()
            MESSAGE(FATAL_ERROR "Please specify QT_VERSION and QWT_URL")
        ENDIF()
    ENDIF()

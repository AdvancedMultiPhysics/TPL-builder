...
IF ( CMAKE_BUILD_UMPIRE )
    ...
    IF ( ENABLE_GXX_DEBUG )
        MESSAGE( WARNING "Umpire and \"-D_GLIBCXX_DEBUG\" do not play nicely with each other" )
    ENDIF()
    IF ( USE_MPI )
    ...
    IF ( UMPIRE_USE_CUDA )
        ...
        # Set more options
        SET( UMPIRE_CONFIGURE_OPTIONS ${UMPIRE_CONFIGURE_OPTIONS} -DCMAKE_CUDA_COMPILER=${CMAKE_CUDA_COMPILER} )
        SET( UMPIRE_CONFIGURE_OPTIONS ${UMPIRE_CONFIGURE_OPTIONS} -DCMAKE_CUDA_HOST_COMPILER=${CMAKE_CXX_COMPILER} )
        SET( UMPIRE_CONFIGURE_OPTIONS ${UMPIRE_CONFIGURE_OPTIONS} -DBLT_CXX_STD=c++${CMAKE_CXX_STANDARD} )
        SET( UMPIRE_CONFIGURE_OPTIONS ${UMPIRE_CONFIGURE_OPTIONS} -DBLT_ENABLE_CUDA=ON )
        IF ( CUDA_TOOLKIT_ROOT_DIR )
            SET( UMPIRE_CONFIGURE_OPTIONS ${UMPIRE_CONFIGURE_OPTIONS} -DCUDA_TOOLKIT_ROOT_DIR=${CUDA_TOOLKIT_ROOT_DIR} )
        ENDIF()
        IF ( CUDA_CUDART_LIBRARY )
            SET( UMPIRE_CONFIGURE_OPTIONS ${UMPIRE_CONFIGURE_OPTIONS} -DCUDA_CUDART_LIBRARY=${CUDA_CUDART_LIBRARY} )
        ENDIF()
        MESSAGE("   UMPIRE configured with cuda:")
    ELSE()
        SET( UMPIRE_CONFIGURE_OPTIONS ${UMPIRE_CONFIGURE_OPTIONS} -DENABLE_CUDA=OFF)
        SET( UMPIRE_CONFIGURE_OPTIONS ${UMPIRE_CONFIGURE_OPTIONS} -DCMAKE_CXX_STANDARD=c++17 )
    ...
    IF ( UMPIRE_USE_HIP )
        ...
        # Set more options
        SET( UMPIRE_CONFIGURE_OPTIONS ${UMPIRE_CONFIGURE_OPTIONS} -DCMAKE_HIP_COMPILER=${CMAKE_HIP_COMPILER} )
        SET( UMPIRE_CONFIGURE_OPTIONS ${UMPIRE_CONFIGURE_OPTIONS} -DCMAKE_HIP_HOST_COMPILER=${CMAKE_CXX_COMPILER} )
        SET( UMPIRE_CONFIGURE_OPTIONS ${UMPIRE_CONFIGURE_OPTIONS} -DCMAKE_HIP_ARCHITECTURES=${CMAKE_HIP_ARCHITECTURES} )
        MESSAGE("   UMPIRE configured with hip:")
    ELSE()
        SET( UMPIRE_CONFIGURE_OPTIONS ${UMPIRE_CONFIGURE_OPTIONS} -DENABLE_HIP=OFF )
        MESSAGE("   UMPIRE configured without hip")
    ENDIF()
    SET( UMPIRE_CONFIGURE_OPTIONS ${UMPIRE_CONFIGURE_OPTIONS} -DENABLE_FORTRAN=ON )
...
FILE( APPEND "${FIND_TPLS_CMAKE}" "    CMAKE_POLICY( SET CMP0146 OLD )\n" )

...
IF ( CMAKE_BUILD_PETSC )
    ...
    IF ( BLAS_LIBS OR LAPACK_LIBS )
        IF ( "${BLAS_LIBS};${LAPACK_LIBS}" MATCHES " " )
            # This is not robust, not sure why it works on some systems and not others
            MESSAGE( WARNING "PETSC does not escape quotes in Blas/Lapack libraies well" )
            SET( PETSC_BLAS_LAPACK ${LAPACK_LIBS} ${BLAS_LIBS} )
            STRING( REPLACE ";" "," PETSC_BLAS_LAPACK "${PETSC_BLAS_LAPACK}")
            STRING( REPLACE "\"" "\\\"" PETSC_BLAS_LAPACK "${PETSC_BLAS_LAPACK}")
            SET( PETSC_BLAS_LAPACK "--with-blaslapack-lib=${PETSC_BLAS_LAPACK}" )
            SET( PETSC_CONFIGURE_OPTIONS ${PETSC_CONFIGURE_OPTIONS} "'${PETSC_BLAS_LAPACK}'" )
        ELSE()
            # This only works when there are no spaces in the path
            SET( PETSC_BLAS_LAPACK ${LAPACK_LIBS} ${BLAS_LIBS} )
            STRING( REPLACE ";" " " PETSC_BLAS_LAPACK "${PETSC_BLAS_LAPACK}")
            STRING( REPLACE "\"" "" PETSC_BLAS_LAPACK "${PETSC_BLAS_LAPACK}")
            SET( PETSC_BLAS_LAPACK "--with-blaslapack-lib='${PETSC_BLAS_LAPACK}'" )
    ...
    IF ( CMAKE_Fortran_COMPILER )
        ...
        ENDIF()
    ELSE()
        SET( PETSC_CONFIGURE_OPTIONS ${PETSC_CONFIGURE_OPTIONS} --with-fc=0 )
    ENDIF()
    ...
    ENDIF()
    IF ( NOT PETSC_PRECISION )
        SET( PETSC_CONFIGURE_OPTIONS ${PETSC_CONFIGURE_OPTIONS} --with-precision=double )
    ELSEIF( "${PETSC_PRECISION}" STREQUAL "single" )  
        SET( PETSC_CONFIGURE_OPTIONS ${PETSC_CONFIGURE_OPTIONS} --with-precision=single )
    ELSEIF( "${PETSC_PRECISION}" STREQUAL "double" )  
        SET( PETSC_CONFIGURE_OPTIONS ${PETSC_CONFIGURE_OPTIONS} --with-precision=double )
    ELSEIF( "${PETSC_PRECISION}" STREQUAL "fp16" )  
        SET( PETSC_CONFIGURE_OPTIONS ${PETSC_CONFIGURE_OPTIONS} --with-precision=__fp16 )
    ELSEIF( "${PETSC_PRECISION}" STREQUAL "fp128" )  
      SET( PETSC_CONFIGURE_OPTIONS ${PETSC_CONFIGURE_OPTIONS} --with-precision=__fp128 )
    ELSE()
        MESSAGE ( FATAL_ERROR "Unknown PETSC_PRECISION=${PETSC_PRECISION}" )
    ENDIF()
    IF ( PETSC_INT64 )
        SET( PETSC_CONFIGURE_OPTIONS ${PETSC_CONFIGURE_OPTIONS} --with-64-bit-indices=1 )
    ENDIF()
    ...
    ENDIF()
MESSAGE( "PETSC_CONFIGURE_OPTIONS=${PETSC_CONFIGURE_OPTIONS}" )
ENDIF()

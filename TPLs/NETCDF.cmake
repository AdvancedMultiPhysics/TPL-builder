...
CONFIGURE_DEPENDENCIES( NETCDF REQUIRED HDF5 )


# Configure netcdf
IF ( CMAKE_BUILD_NETCDF )
    SET( NETCDF_CONFIGURE_OPTIONS )
    SET( NETCDF_CONFIGURE_OPTIONS ${NETCDF_CONFIGURE_OPTIONS} --prefix=${NETCDF_INSTALL_DIR} )
    SET( NETCDF_CONFIGURE_OPTIONS ${NETCDF_CONFIGURE_OPTIONS} --with-hdf5=${HDF5_INSTALL_DIR} )
    SET( NETCDF_CONFIGURE_OPTIONS ${NETCDF_CONFIGURE_OPTIONS} --disable-examples --enable-netcdf4 --disable-dap )
    ...
    SET( NETCDF_LIBS -ldl )
    IF ( ENABLE_SHARED )
        SET( NETCDF_CONFIGURE_OPTIONS ${NETCDF_CONFIGURE_OPTIONS} --enable-shared )
    ELSE()
        SET( NETCDF_CONFIGURE_OPTIONS ${NETCDF_CONFIGURE_OPTIONS} --disable-shared )
    ENDIF()
    IF ( ENABLE_STATIC )
        SET( NETCDF_CONFIGURE_OPTIONS ${NETCDF_CONFIGURE_OPTIONS} --enable-static )
    ELSE()
...
FILE( APPEND "${FIND_TPLS_CMAKE}" "IF ( TPLs_FIND_NETCDF AND NOT TPLs_NETCDF_FOUND )\n" )
...
FILE( APPEND "${FIND_TPLS_CMAKE}" "    ADD_TPL_LIBRARY( NETCDF $\{NETCDF_LIB} )\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "ENDIF()\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "\n" )

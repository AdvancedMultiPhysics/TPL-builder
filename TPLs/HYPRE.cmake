...
ENDIF()
IF ( ( NOT DEFINED HYPRE_ENABLE_GPU_AWARE_MPI ) AND ( HYPRE_USE_CUDA OR HYPRE_USE_HIP ) )
    SET( HYPRE_ENABLE_GPU_AWARE_MPI ${ENABLE_GPU_AWARE_MPI} )
ENDIF()
IF ( NOT DEFINED HYPRE_USE_OPENMP )
    SET( HYPRE_USE_OPENMP ${USE_OPENMP} )
ENDIF()
IF ( NOT DEFINED HYPRE_CXX_STD )
    SET( HYPRE_CXX_STD ${CXX_STD} )
ENDIF()
...
IF ( CMAKE_BUILD_HYPRE )
    ...
    IF( HYPRE_USE_SINGLE_PRECISION )
        ADD_HYPRE_OPTION( --enable-single )
    ENDIF()
    # enable once debugged  
    # ADD_HYPRE_OPTION( --with-cxxstandard=${HYPRE_CXX_STD} )      
    ...
    IF( HYPRE_USE_CUDA )
        ...
        ELSEIF ( CMAKE_CUDA_ARCHITECTURES )
            ADD_HYPRE_OPTION( --with-gpu-arch=${CMAKE_CUDA_ARCHITECTURES} )
        ELSE()
    ...
    IF( HYPRE_USE_HIP )
        ...
        # Async memcpy bug only needs patching on gfx942 architectures fixed for >= 3.1.0 with rocm >= 5.6.1
        IF (( HYPRE_HIP_ARCH STREQUAL "gfx942" ) AND ( HYPRE_VERSION VERSION_LESS "3.1.0" ))
            SET( HYPRE_PATCH_COMMAND patch -p2 -i "${CMAKE_CURRENT_SOURCE_DIR}/patches/hypre.memcpyasync.patch" )
        ENDIF()
        IF (HYPRE_VERSION VERSION_EQUAL "3.1.0")
            SET( HYPRE_PATCH_COMMAND patch -p2 -i "${CMAKE_CURRENT_SOURCE_DIR}/patches/hypre.hipidentity.patch" )
        ENDIF()
    ENDIF()

    IF( HYPRE_ENABLE_GPU_AWARE_MPI )
      ADD_HYPRE_OPTION( --enable-gpu-aware-mpi )
    ENDIF()
      
    IF( HYPRE_USE_CUDA OR HYPRE_USE_HIP )
        IF ( NOT DEFINED HYPRE_DISABLE_UNIFIED_MEMORY )
        ...
        IF ( HYPRE_USE_UMPIRE )
            ...
            ELSE()
                ADD_HYPRE_OPTION( --with-umpire-device )
            ENDIF()

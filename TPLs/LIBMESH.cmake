...
CONFIGURE_DEPENDENCIES( LIBMESH REQUIRED OPTIONAL METIS PARMETIS GKLIB )
...
IF ( CMAKE_BUILD_LIBMESH )
    ...
    IF ( ENABLE_GXX_DEBUG ) 
        ...
        SET( LIBMESH_CONFIGURE_OPTIONS ${LIBMESH_CONFIGURE_OPTIONS} --enable-glibcxx-debugging-cppunit=yes )
    ELSE()
    ...
    SET( LIBMESH_CONFIGURE_OPTIONS ${LIBMESH_CONFIGURE_OPTIONS} --enable-netcdf  )
    ...
    SET( LIBMESH_CONFIGURE_OPTIONS ${LIBMESH_CONFIGURE_OPTIONS} --disable-netgen    )
    ...
    SET( LIBMESH_CONFIGURE_OPTIONS ${LIBMESH_CONFIGURE_OPTIONS} --disable-qhull     )
    SET( LIBMESH_CONFIGURE_OPTIONS ${LIBMESH_CONFIGURE_OPTIONS} --disable-vsmoother )
    SET( LIBMESH_CONFIGURE_OPTIONS ${LIBMESH_CONFIGURE_OPTIONS} --disable-metaphysicl )
    # SET( LIBMESH_CONFIGURE_OPTIONS ${LIBMESH_CONFIGURE_OPTIONS} --disable-amr       ) # Causes issues
    IF ( METIS_CONFIGURED )
        SET( LIBMESH_CONFIGURE_OPTIONS ${LIBMESH_CONFIGURE_OPTIONS} --with-metis=${METIS_INSTALL_DIR}/lib )
        SET( LIBMESH_CONFIGURE_OPTIONS ${LIBMESH_CONFIGURE_OPTIONS} --with-metis-include=${METIS_INSTALL_DIR}/include )
        SET( LIBMESH_IGNORE_LINK_ERRORS -i )
    ELSE()
        SET( LIBMESH_CONFIGURE_OPTIONS ${LIBMESH_CONFIGURE_OPTIONS} --with-metis=internal )
    ENDIF()

    SET( LIBMESH_CONFIGURE_OPTIONS ${LIBMESH_CONFIGURE_OPTIONS} --enable-parmetis   )
    IF ( PARMETIS_CONFIGURED )
        SET( LIBMESH_CONFIGURE_OPTIONS ${LIBMESH_CONFIGURE_OPTIONS} --with-parmetis=${PARMETIS_INSTALL_DIR}/lib )
        SET( LIBMESH_CONFIGURE_OPTIONS ${LIBMESH_CONFIGURE_OPTIONS} --with-parmetis-include=${PARMETIS_INSTALL_DIR}/include )
        SET( LIBMESH_IGNORE_LINK_ERRORS -i )
    ELSE()
        SET( LIBMESH_CONFIGURE_OPTIONS ${LIBMESH_CONFIGURE_OPTIONS} --with-parmetis=internal )
    ENDIF()
    ...
    ENDIF()
    SET( LIBMESH_CXX_FLAGS "${LIBMESH_CXX_FLAGS} ${ENV_CXX_FLAGS}" )
    ...
    ADD_TPL( 
        ...
        BUILD_IN_SOURCE     0
        INSTALL_COMMAND     $(MAKE) install ${LIBMESH_IGNORE_LINK_ERRORS}
...
FILE( APPEND "${FIND_TPLS_CMAKE}" "IF ( TPLs_FIND_LIBMESH AND NOT TPLs_LIBMESH_FOUND )\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "    INCLUDE( \"${CMAKE_INSTALL_PREFIX}/cmake/FindLibmesh.cmake\" )\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "    SET( LIBMESH_INSTALL_DIR \"${LIBMESH_INSTALL_DIR}\" )\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "    FIND_PACKAGE( OpenMP )\n" )  # Libmesh disable-openmp does not actually disable OpenMP
FILE( APPEND "${FIND_TPLS_CMAKE}" "    LIBMESH_SET_INCLUDES( $\{LIBMESH_INSTALL_DIR} )\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "    LIBMESH_SET_LIBRARIES( $\{LIBMESH_INSTALL_DIR} )\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "    ADD_TPL_LIBRARY( LIBMESH $\{LIBMESH_LIBS} )\n" )

...
IF ( CMAKE_BUILD_THRUST )
    ...
    SET( THRUST_CONFIGURE_OPTIONS ${THRUST_CONFIGURE_OPTIONS} -DCMAKE_C_COMPILER=${CMAKE_C_COMPILER} )
    SET( THRUST_CONFIGURE_OPTIONS ${THRUST_CONFIGURE_OPTIONS} -DCMAKE_CXX_COMPILER=${CMAKE_CXX_COMPILER} )
...
FILE( APPEND "${FIND_TPLS_CMAKE}" "IF ( TPLs_FIND_THRUST AND NOT TPLs_THRUST_FOUND )\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "    SET( Thrust_DIR \"${THRUST_CMAKE_CONFIG_DIR}\" )\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "    SET( THRUST_USE_CUDA \"${THRUST_USE_CUDA}\" )\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "    SET( THRUST_USE_OPENMP \"${THRUST_USE_OPENMP}\" )\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "    FIND_PACKAGE( Thrust REQUIRED CONFIG )\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "    IF (THRUST_USE_CUDA)\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "        thrust_create_target(Thrust)\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "    ELSEIF(THRUST_USE_OPENMP)\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "        thrust_create_target(Thrust DEVICE OMP)\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "    ELSE()\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "        thrust_create_target(Thrust DEVICE CPP)\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "    ENDIF()\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "    SET( TPL_LIBRARIES $\{TPL_LIBRARIES} Thrust )\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "    SET( TPLs_THRUST_FOUND TRUE )\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "ENDIF()\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "\n" )

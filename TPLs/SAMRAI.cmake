...
CONFIGURE_DEPENDENCIES( SAMRAI REQUIRED LAPACK OPTIONAL ZLIB HDF5 UMPIRE RAJA )
# CONFIGURE_DEPENDENCIES( SARMAI REQUIRED LAPACK OPTIONAL ZLIB HDF5 UMPIRE RAJA HYPRE PETSC SILO SUNDIALS CONDUIT )
...
IF ( NOT DEFINED SAMRAI_USE_UMPIRE )
    SET( SAMRAI_USE_UMPIRE ${UMPIRE_CONFIGURED} )
ENDIF()
IF ( NOT DEFINED SAMRAI_USE_RAJA )
    SET( SAMRAI_USE_RAJA ${RAJA_CONFIGURED} )
ENDIF()
IF ( CMAKE_BUILD_SAMRAI )
    ...
    IF ( NOT CMAKE_Fortran_COMPILER )
        MESSAGE( FATAL_ERROR "SAMRAI requires Fortran" )
    ENDIF()
    IF ( ENABLE_GXX_DEBUG )
        MESSAGE( WARNING "SAMRAI has some errors when compiling with \"-D_GLIBCXX_DEBUG\" (exercised by SAMRSolvers)" )
    ENDIF()
    ...
    SET( SAMRAI_CONFIGURE_OPTIONS ${SAMRAI_CONFIGURE_OPTIONS} -DCMAKE_CXX_STANDARD=${CXX_STD} )
    SET( SAMRAI_CONFIGURE_OPTIONS ${SAMRAI_CONFIGURE_OPTIONS} -DBLT_CXX_STD=c++${CMAKE_CXX_STANDARD} )
    IF ( SAMRAI_USE_CUDA )
        MESSAGE( "Enabling CUDA support for SAMRAI" )
        IF ( CUDA_TOOLKIT_ROOT_DIR )
          SET( SAMRAI_CONFIGURE_OPTIONS ${SAMRAI_CONFIGURE_OPTIONS} -DCUDA_TOOLKIT_ROOT_DIR=${CUDA_TOOLKIT_ROOT_DIR} )
        ENDIF()
...
CONFIGURE_FILE( ${CMAKE_CURRENT_SOURCE_DIR}/cmake/FindSAMRAI.cmake "${CMAKE_INSTALL_PREFIX}/cmake/FindSAMRAI.cmake" COPYONLY )
FILE( APPEND "${FIND_TPLS_CMAKE}" "\n# Find SAMRAI\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "IF ( TPLs_FIND_SAMRAI AND NOT TPLs_SAMRAI_FOUND )\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "    SET( CMAKE_MODULE_PATH \"${CMAKE_INSTALL_PREFIX}/cmake\" $\{CMAKE_MODULE_PATH} )\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "    SET( SAMRAI_INSTALL_DIR \"${SAMRAI_INSTALL_DIR}\" )\n" )
FILE( APPEND "${FIND_TPLS_CMAKE}" "    FIND_PACKAGE( SAMRAI REQUIRED )\n" )

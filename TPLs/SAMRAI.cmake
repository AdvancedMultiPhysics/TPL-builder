...
IF ( CMAKE_BUILD_SAMRAI )
    ...
    SET( SAMRAI_CONFIGURE_OPTS ${SAMRAI_CONFIGURE_OPTS} -DCMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} )
    # Set third party library includes
    SET( SAMRAI_DEPENDS LAPACK HDF5 ZLIB )
    IF ( SAMRAI_USE_UMPIRE )
        SET( SAMRAI_DEPENDS UMPIRE ${SAMRAI_DEPENDS} )
	MESSAGE( "Building SAMRAI with Umpire support" )
        SET( CONFIGURE_OPTIONS ${CONFIGURE_OPTIONS}  -DENABLE_UMPIRE=ON  )
	SET( CONFIGURE_OPTIONS ${CONFIGURE_OPTIONS}  -Dumpire_DIR=${UMPIRE_INSTALL_DIR}/share/umpire/cmake )
    ENDIF()
    IF ( SAMRAI_USE_RAJA )
        SET( SAMRAI_DEPENDS RAJA ${SAMRAI_DEPENDS} )
	MESSAGE( "Building SAMRAI with RAJA support" )
        SET( CONFIGURE_OPTIONS ${CONFIGURE_OPTIONS}  -DENABLE_RAJA=ON  )
	SET( CONFIGURE_OPTIONS ${CONFIGURE_OPTIONS}  -DRAJA_DIR=${RAJA_INSTALL_DIR}/share/raja/cmake )
    ENDIF()
    MESSAGE( "SAMRAI_DEPENDS ${SAMRAI_DEPENDS}" )  
    IF ( USE_MPI )
    ...
    IF ( ${index} GREATER -1 )
        SET( SAMRAI_DEPENDS ${SAMRAI_DEPENDS} TIMER )
    ENDIF()
    SET( SAMRAI_CONFIGURE_OPTS ${SAMRAI_CONFIGURE_OPTS} -DENABLE_CPPCHECK=OFF )
    ...
    # Enable the tests
    IF ( SAMRAI_TEST )
	    SET( SAMRAI_CONFIGURE_OPTS ${SAMRAI_CONFIGURE_OPTS} -DENABLE_SAMRAI_TESTS=ON )
    ELSE()
	SET( SAMRAI_CONFIGURE_OPTS ${SAMRAI_CONFIGURE_OPTS} -DENABLE_SAMRAI_TESTS=OFF )
    ENDIF()
    IF ( SAMRAI_TOOLS )
    ...
    ENDIF()
    MESSAGE("   SAMRAI configure options: ${SAMRAI_CONFIGURE_OPTS}")
ENDIF()

...
int Lapack::run_all_test()
{
    ...
    if ( test_dcopy( N, error ) ) {
        printf( "test_dcopy failed (%e)\n", error );
        N_errors++;
    }
    if ( test_dnrm2( N, error ) ) {
        printf( "test_dnrm2 failed (%e)\n", error );
        N_errors++;
    }
    if ( test_daxpy( N, error ) ) {
        printf( "test_daxpy failed (%e)\n", error );
        N_errors++;
    }
    if ( test_dasum( N, error ) ) {
        printf( "test_dasum failed (%e)\n", error );
        N_errors++;
    }
    if ( test_ddot( N, error ) ) {
        printf( "test_ddot failed (%e)\n", error );
        N_errors++;
    }
    // Matrix blas operations
    if ( test_dgemv( N, error ) ) {
        printf( "test_dgemv failed (%e)\n", error );
        N_errors++;
    }
    if ( test_dgemm( N, error ) ) {
        printf( "test_dgemm failed (%e)\n", error );
        N_errors++;
    }
    // Linear solves
    if ( test_dgesv( N, error ) ) {
        printf( "test_dgesv failed (%e)\n", error );
        N_errors++;
    }
    if ( test_dgtsv( N, error ) ) {
        printf( "test_dgtsv failed (%e)\n", error );
        N_errors++;
    }
    if ( test_dgbsv( N, error ) ) {
        printf( "test_dgbsv failed (%e)\n", error );
        N_errors++;
    }
    // Linear factorizations
    if ( test_dgetrf( N, error ) ) {
        printf( "test_dgetrf failed (%e)\n", error );
        N_errors++;
    }
    if ( test_dgttrf( N, error ) ) {
        printf( "test_dgttrf failed (%e)\n", error );
        N_errors++;
    }
    if ( test_dgbtrf( N, error ) ) {
        printf( "test_dgbtrf failed (%e)\n", error );
        N_errors++;
    }
    // Solve using factorization
    if ( test_dgetrs( N, error ) ) {
        printf( "test_dgetrs failed (%e)\n", error );
        N_errors++;
    }
    if ( test_dgttrs( N, error ) ) {
        printf( "test_dgttrs failed (%e)\n", error );
        N_errors++;
    }
    if ( test_dgbtrs( N, error ) ) {
        printf( "test_dgbtrs failed (%e)\n", error );
        N_errors++;
    }
    // Inverse using factorization
    if ( test_dgetri( N, error ) ) {
        printf( "test_dgetri failed (%e)\n", error );
        N_errors++;
    }
    return N_errors > 0;
}
...
static inline void random( int N, double *data )
{
    ...
    for ( int i = 0; i < N; i++ )
        data[i] =
            static_cast<double>( rand() ) / rmax + 1e-7 * static_cast<double>( rand() ) / rmax;
}


// Check if two vectors are approximately equal
static inline bool approx_equal(
    int N, const double *x1, const double *x2, const double tol = 1e-12 )
{
    bool pass = true;
    for ( int i = 0; i < N; i++ )
...
void Lapack::print_machine_parameters()
{
    ...
    printf(
        "rnd   = %13.6e    1.0 when rounding occurs in addition, 0.0 otherwise\n", dlamch( 'R' ) );
...
static int LapackWrappers_lock_queue_error =
    pthread_mutex_init( &LapackWrappers_lock_queue, nullptr );
#else
#error Not programmed
#endif
#ifdef WINDOWS
void Lapack::get_lock() { WaitForSingleObject( LapackWrappers_lock_queue, INFINITE ); }
#elif defined( LINUX )

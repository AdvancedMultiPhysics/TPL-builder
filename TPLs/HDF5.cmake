...
IF ( CMAKE_BUILD_HDF5 )
    ...
    IF ( HDF5_ENABLE_CXX )
        ...
        SET( HDF5_CONFIGURE_OPTIONS ${HDF5_CONFIGURE_OPTIONS} --enable-cxx )
    ELSE()
        SET( HDF5_CONFIGURE_OPTIONS ${HDF5_CONFIGURE_OPTIONS} --disable-cxx )
    ENDIF()
    IF ( HDF5_ENABLE_UNSUPPORTED )
        SET( HDF5_CONFIGURE_OPTIONS ${HDF5_CONFIGURE_OPTIONS} --enable-unsupported )
    ENDIF()
    IF ( USE_MPI )
        SET( HDF5_CONFIGURE_OPTIONS ${HDF5_CONFIGURE_OPTIONS} --enable-parallel )
        IF ( MPIEXEC )
            SET( HDF5_CONFIGURE_OPTIONS ${HDF5_CONFIGURE_OPTIONS} RUNPARALLEL=${MPIEXEC} )
        ELSE()
            SET( HDF5_CONFIGURE_OPTIONS ${HDF5_CONFIGURE_OPTIONS} RUNPARALLEL=NONE )
        ENDIF()
    ELSE()
        SET( HDF5_CONFIGURE_OPTIONS ${HDF5_CONFIGURE_OPTIONS} --disable-parallel )
    ENDIF()
    ...
    IF ( HDF5_ENABLE_FORTRAN )
        ...
        SET( HDF5_CONFIGURE_OPTIONS ${HDF5_CONFIGURE_OPTIONS} --enable-fortran )
    ENDIF()
    SET( HDF5_CONFIGURE_OPTIONS ${HDF5_CONFIGURE_OPTIONS} --with-zlib='${ZLIB_INCLUDE_DIR}','${ZLIB_LIB_DIR}' )
    SET( HDF5_CONFIGURE_OPTIONS ${HDF5_CONFIGURE_OPTIONS} --includedir=${HDF5_INSTALL_DIR}/include )
    SET( HDF5_CONFIGURE_OPTIONS ${HDF5_CONFIGURE_OPTIONS} --libdir=${HDF5_INSTALL_DIR}/lib )
    IF ( NOT HDF5_VERSION )
    ...
    ELSE()
        # Older HDF5 version
        IF ( ${CMAKE_BUILD_TYPE} STREQUAL "Debug" )
            SET(HDF5_CONFIGURE_OPTIONS ${HDF5_CONFIGURE_OPTIONS} --disable-production --enable-debug )
        ELSEIF ( ${CMAKE_BUILD_TYPE} STREQUAL "Release" )
            SET(HDF5_CONFIGURE_OPTIONS ${HDF5_CONFIGURE_OPTIONS} --enable-production --disable-debug )
        ELSE()
            MESSAGE ( FATAL_ERROR "Unknown CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE}" )
        ENDIF()
    ENDIF()
    IF ( ENABLE_SHARED )
        SET( HDF5_CONFIGURE_OPTIONS ${HDF5_CONFIGURE_OPTIONS} --enable-shared )
        ...
        ENDIF()
    ELSE()
        SET( HDF5_CONFIGURE_OPTIONS ${HDF5_CONFIGURE_OPTIONS} --disable-shared )
    ENDIF()
    IF ( ENABLE_STATIC )
        SET( HDF5_CONFIGURE_OPTIONS ${HDF5_CONFIGURE_OPTIONS} --enable-static )
    ELSE()
        SET( HDF5_CONFIGURE_OPTIONS ${HDF5_CONFIGURE_OPTIONS} --disable-static )
    ENDIF()
    SET( HDF5_CONFIGURE_OPTIONS ${HDF5_CONFIGURE_OPTIONS} --enable-tests=no )
    IF ( BUILD_TYPE )
        SET( HDF5_CONFIGURE_OPTIONS ${HDF5_CONFIGURE_OPTIONS} --build=${BUILD_TYPE} )
    ENDIF()
ELSE()
    SET( HDF5_COMPONENTS "C" )
